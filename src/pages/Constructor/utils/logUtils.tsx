...
/**
 * Renders the appropriate log level indicator with proper styling
 * @param content Log content
 * @returns React element with styled log level
 */
export const renderLogLevel = (content: string) => {
  if (content.includes(`[${LOG_LEVELS.INFO}]`)) {
    return <span className="text-blue-500 font-medium">[INFO]</span>;
  } else if (content.includes(`[${LOG_LEVELS.WARNING}]`)) {
    return <span className="text-yellow-500 font-medium">[WARNING]</span>;
  } else if (content.includes(`[${LOG_LEVELS.ERROR}]`)) {
    return <span className="text-red-500 font-medium">[ERROR]</span>;
  } else if (content.includes(`[${LOG_LEVELS.DEBUG}]`)) {
